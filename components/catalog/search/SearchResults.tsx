...
export default function SearchResults({
  ...
  return (
    <div className="absolute z-40 mt-2 w-full rounded-lg border border-secondary-500 bg-secondary-600 shadow-lg">
      {isLoading ? (
        <p className="px-4 py-3 text-sm text-secondary-200">{copy.search.searching}</p>
      ) : isError ? (
        <p className="px-4 py-3 text-sm text-red-600">{error ?? copy.search.failed}</p>
      ) : results.length === 0 ? (
        <p className="px-4 py-3 text-sm text-secondary-200">{copy.general.noResultsLabel}</p>
      ) : (

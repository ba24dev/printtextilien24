...
export default function ProductBreadcrumbs({ product }: ProductBreadcrumbsProps) {
    const collections = product.collections?.nodes?.filter((node) => node && !node.handle?.startsWith("hidden")) ?? [];

    return (
        <nav aria-label="Breadcrumb" className="text-xs text-foreground/60">
            <ol className="flex flex-wrap items-center gap-2">
                <li>
                    <Link href="/products" className="font-medium transition hover:text-primary-200">
                        Shop
                    </Link>
                </li>
